...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            ...
            <span class="font-mono text-sm text-gray-500">{{ $account->account_code }}</span>
        </div>
        
        <form action="{{ route('admin.accounts.update', $account->id) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Account Code</label>
                <input type="text" name="account_code" value="{{ $account->account_code }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Account Name</label>
                <input type="text" name="account_name" value="{{ $account->account_name }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
                ...
                <div>
                    ...
                    <select name="parent_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" {{ $account->parent_id == $parent->id ? 'selected' : '' }}>{{ $parent->account_code }} - {{ $parent->account_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="account_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @foreach(['asset', 'liability', 'equity', 'revenue', 'expense'] as $type)
                            <option value="{{ $type }}" {{ $account->account_type == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <select name="sub_ledger_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <option value="customer" {{ $account->sub_ledger_type == 'customer' ? 'selected' : '' }}>Customer (AR)</option>
                    <option value="supplier" {{ $account->sub_ledger_type == 'supplier' ? 'selected' : '' }}>Supplier (AP)</option>
                </select>
