...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.roles.update', $role) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="name" id="name" class="w-full max-w-md border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('name', $role->name) }}" required {{ $role->name  == 'Store Admin' ? 'readonly' : '' }}>
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
            @if($role->name == 'Store Admin')
                <p class="mt-1 text-xs text-gray-500">System permissions cannot be modified for Store Admin.</p>
            @endif
        </div>
        ...
        <div class="mb-6">
            ...
            @if($role->name == 'Store Admin')
                <div class="bg-green-50 p-4 rounded text-green-700">
                    Store Admin has full access to all modules.
                </div>
            @else
        ...
        <div class="flex justify-end pt-4 border-t">
            ...
            <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 transition shadow-lg" {{ $role->name == 'Store Admin' ? 'disabled' : '' }}>Update Role</button>
        </div>
    </form>
</div>
@endsection
