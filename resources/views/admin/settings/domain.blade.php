...
<div class="max-w-4xl mx-auto py-6">
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('admin.settings.index') }}" class="text-indigo-600 hover:text-indigo-800 font-medium">← Back to Settings</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        ...
        <div class="p-6">
            ...
            <div class="flex items-center space-x-2">
                ...
                <a href="{{ request()->getSchemeAndHttpHost() }}" target="_blank" class="p-2 text-gray-500 hover:text-indigo-600 transition">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            @if($currentRequest)
                ...
                <div class="border rounded-xl p-6 {{ $currentRequest->status === 'approved' ? 'border-green-200 bg-green-50' : 'border-yellow-200 bg-yellow-50' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="font-bold text-lg {{ $currentRequest->status === 'approved' ? 'text-green-800' : 'text-yellow-800' }}">
                                {{ $currentRequest->domain }}
                            </h4>
                            <div class="mt-2 flex items-center space-x-2">
                                <span class="capitalize px-3 py-1 rounded-full text-xs font-bold {{ $currentRequest->status === 'approved' ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800' }}">
                                    {{ $currentRequest->status }}
                                </span>
                                @if($currentRequest->status === 'pending')
                                    <span class="text-sm text-gray-500">Submitted on {{ $currentRequest->created_at->format('M d, Y') }}</span>
                                @endif
                            </div>
                        </div>
                        
                        @if($currentRequest->status === 'pending')
                        <form action="{{ route('admin.settings.domain.cancel', $currentRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this request?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-800 font-medium text-sm underline">Cancel Request</button>
                        </form>
                        @endif
                    </div>
            ...
            @else
                <!-- New Request Form -->
                <form action="{{ route('admin.settings.domain.request') }}" method="POST">
                    @csrf
                    <div class="mb-6">
                        <label for="domain" class="block text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">Domain Name</label>
                        <div class="relative rounded-md shadow-sm">
                            <input type="text" name="domain" id="domain" class="block w-full rounded-xl border-3 border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 py-3 pl-4 pr-12 sm:text-sm" placeholder="e.g. myshop.com" value="{{ old('domain') }}">
                        </div>
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
                        ...
                        <p class="text-sm text-blue-800">
                            ...
                            <code class="bg-blue-100 px-1 py-0.5 rounded font-mono">{{ parse_url(config('app.url'), PHP_URL_HOST) }}</code> in your domain's DNS settings.
                        </p>
                    </div>

                    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl shadow-md transition transform hover:-translate-y-0.5">
                        Connect Domain
                    </button>
                </form>
            @endif
