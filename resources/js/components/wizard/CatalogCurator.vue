<template>
    <div class="space-y-4">
        <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
            ...
            <ul class="space-y-4">
                <li v-for="category in localCategories" :key="category.id" class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4 flex-1">
                            <!-- Helper for drag handle if we implemented drag and drop, currently just visual -->
                            ...
                            <div class="flex flex-col flex-1">
                                <span class="text-base font-bold text-[#0A2540]">{{ category.name }}</span>
                                <div v-if="category.is_visible_online" class="mt-2 text-sm">
                                     <input 
                                        type="text" 
                                        v-model="category.public_display_name"
                                        class="w-full max-w-xs text-sm border-2 border-gray-100 bg-gray-50 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#0A2540]/10 focus:border-[#0A2540] transition-all"
                    ...
                    <!-- Children -->
                    <ul v-if="category.children && category.children.length > 0 && category.is_visible_online" class="ml-10 mt-6 space-y-3 border-l-2 border-gray-50 pl-6">
                         <li v-for="child in category.children" :key="child.id" class="flex items-center justify-between py-2 group">
                                <div class="flex items-center gap-3">
                                    <span class="text-sm font-medium text-gray-600">{{ child.name }}</span>
                                    <input 
                                        type="text" 
                                        v-model="child.public_display_name"
                                        v-show="child.is_visible_online"
                                        class="ml-2 text-xs border-2 border-gray-100 bg-gray-50 rounded-lg px-3 py-1 focus:ring-2 focus:ring-[#0A2540]/10 focus:border-[#0A2540] transition-all"
...
const localCategories = ref(props.modelValue);

watch(() => props.modelValue, (newVal) => {
    localCategories.value = newVal;
}, { deep: true });

watch(localCategories, (newVal) => {
